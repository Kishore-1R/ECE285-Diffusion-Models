...
def run_inpainting(
    ...
    Args:
        ...
        image_path: Path to the original image (if None, will look in output_dir)
        mask_path: Path to the mask image (if None, will look in output_dir)
    """
    ...
    image = (
        torch.from_numpy(image).permute(2, 0, 1).unsqueeze(0).half()
    )  # Convert to float16
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run inpainting with resume capability"
    )
    parser.add_argument(
        "--resume_image", type=str, help='Resume from this image number (e.g., "07")'
    )
...
def main():
    ...
    if args.resume_image and args.resume_u:
        ...
        for idx, config in enumerate(configs):
            ...
            if (
                img_num == int(args.resume_image)
                and config["U"] == args.resume_u
                and config["sampler"] == args.resume_sampler
            ):
                resume_idx = idx
                break
