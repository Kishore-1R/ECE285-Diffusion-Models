...
def plot_inpainting_results(base_dir="results/inpainting_tests"):
    ...
    n_cols = 6  # original + original with mask + 4 different U values
    ...
    for row, folder in enumerate(image_folders):
        ...
        # Plot results for different U values
        for i, u_val in enumerate([1, 2, 5, 10]):
            result_path = os.path.join(folder, f"ddpm_U{u_val}", "inpainted.png")
            if os.path.exists(result_path):
                result = Image.open(result_path)
                plt.subplot(n_rows, n_cols, row * n_cols + i + 3)
                plt.imshow(result)
                if row == 0:
                    plt.title(f"U={u_val}", fontsize=20, pad=15)
                plt.axis("off")
            else:
    ...
    # Save the figure
    output_path = os.path.join(base_dir, "ddpm_inpainting_comparison.png")
